...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.header={
            ...
            'DNT': '1',
            'Host': 'uos.starpark.me',
            'Referer': 'https://www.bimiacg4.net/bangumi/',
    ...
    def pushButton_Click_Search(self):
        ...
        if re.compile(r'www.bimiacg4.net').findall(URL)!=[]:
            Referer='https://www.bimiacg4.net/bangumi/'+Referer
        ...
        try:
            ...
            uid=re.sub('"url":"', '', uid)
            VideoHtml='https://www.bimiacg4.net/static/danmu/play.php?url='+uid+'&myurl='+URL
            
            r = requests.get(VideoHtml,headers=self.header2,verify=False)
            soup = BeautifulSoup(r.text,"html.parser")
            script = soup.select("video")
            uid=re.compile(r'src=".+"').findall(str(script))[0]
            uid=re.sub('" type.+', '', uid)
            VideoHtml2=re.sub('src="', '', uid)
            self.ui.lineEdit_2.setText(VideoHtml2)
